...
最近は、社内にデータ分析や統計を普及させるために輪読会などを開いたりしています。

本記事では、アクセスログを分析して施策立案や機能改善に活かしたいという時に「行動時系列」という以下のような図示の仕方がとても良かったので、それの紹介をします！
...
これもよく読めば分析できそうですが、パターンを捉えるには難しいと思います。
...
時間帯ごとに１つのデータになるように今回は1時間ごとにログをまとめます。
`groupby`を用いて同じ1時間内のログを１つに集計します。
...
この図示の欠点としては、時間解像度を細かくしたり行動のカテゴリを増やすほど色を見分けづらくなります。自分はこれを解消するために図の拡大・収縮ができるPlotlyを利用しています。あとは、大画面に映せると見やすいです。
